...
function crearNuevaSeccion(categoria, productos) {
    ...
    const productHeaderContent = `<h2>${categoria}</h2>
        <a class="link" href="products.html?q="${''}>Ver todo <i class="fa-solid fa-arrow-right"></i></a>`;
    ...
    productos.forEach(producto => {
        ...
        cards += `
            <div class="products-card">
                <img class="products-img" src="${imagen}" alt="">
                <h4>${nombre}</h4>
                <p class="products-price">$${precio}</p>
                <a class="link" href="product-detail.html?id=${id}" data-id="${id}">Ver producto</a>
            </div>`
    })
